...
interface IZKCService
{
    ...
    * @return 结果
    */
    boolean showSwitch(in boolean flg,in int brightness);

    /**
    * 显示字符串，支持UTF-8
    * @return 结果
    */
    boolean showString(in String textStr,in int x,in int y,in int w,in int h,in int fontsize,in String fontcolor,in String align);

    /**
    * 显示二维码
    * @return 结果
    */
    boolean showQrCode(in String qrStr, in int width,in  int x,in  int y);

    /**
    * 显示图片
    * @return 结果
    */
    boolean showImage(in Bitmap bitmap,in  int x,in  int y,in  int w,in  int h, in int rotate,in  int quality);

    /**
    * 显示灰度图片
    * @return 结果
    */
    boolean showGrayImage(in Bitmap bitmap, in int x, in int y, in int w, in int h, in int rotate, in int quality);

    //区域清除
    boolean showCleanScreen(in int x,in int y,in int w,in int h);

    //全屏清除
    boolean showCleanAllScreen();

    //清空指定填充
    boolean showFillColor(in int x,in int y,in int w,in int h,in String colorStr);

    //全屏填充
    boolean showFillScreen(in String colorStr);
}