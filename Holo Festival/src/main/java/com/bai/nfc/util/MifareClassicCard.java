...
public class MifareClassicCard {
	...
	/**
	 * 返回标签字节的大??Return the size of the tag in bytes.
	 * 
	 * @return
	 */
	public int getSize() {
	...
	/**
	 * Return the first block of a given sector. 返回指定扇区的第????
	 * 
	 * @param sectorIndex
	 * @return
	 */
	...
	/**
	 * Send raw NfcA data to a tag and receive the response. 原始数据发??到卡和接受响??
	 * 
	 * @param data
	 * @return
	 */
	...
	/**
	 * Copy from the temporary block to a value block. 从临时快赋????????
	 * 
	 * @param blockIndex
	 */
	...
	public String wirteCarCode(String carCode, int block, String key) {
		...
		try {
			if (block % 4 == 3) {
				string = " This block is a secret key block ";
				return string;
			}
			connect();
	...
	public String modifyPassword(int block, String newPassword, String key) {
		...
		try {
			if (block % 4 != 3) {
				string = " This block is not a key block ！";
				return string;
			}
